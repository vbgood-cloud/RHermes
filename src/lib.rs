...
//!
//! 这是库入口，提供三个应用入口：
//! - `run_default_app()` — 通用模式（原 main）
//! - `run_student_app()` — 学生模式
//! - `run_teacher_app()` — 教师模式
...
/// 通用子命令集（rhermes / rhermes-stu / rhermes-teacher 都可用）
#[derive(Subcommand)]
pub enum CommonCommands {
    ...
    /// 🎓 教育模式（透传形式：rhermes edu <student|teacher|auth|...> [...]）
    Edu {
        /// 教育子命令及参数（透传给 edu::handle_edu）
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
}

// ── 教师 CLI（rhermes-teacher = 通用 + 教师特有）──────────────────────────

#[derive(Parser)]
#[command(name = "rhermes-teacher")]
#[command(about = "RHermes 教师版 — 通用能力 + 教学管理", long_about = None)]
#[command(version)]
pub struct TeacherCli {
    /// 恢复上一次会话内容
    #[arg(short = 'r', long)]
    resume: bool,

    #[command(subcommand)]
    command: Option<TeacherCommands>,
}

#[derive(Subcommand)]
pub enum TeacherCommands {
    // 通用子命令（flatten）
    #[command(flatten)]
    Common(CommonCommands),
    // 教师特有
    /// 📚 课程管理 (create <课程码> <课程名> | list)
    Course {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    ...
    /// 📅 课次管理 (create <课程码> <班级> <序号> <主题>)
    Lesson {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    /// 👨‍🎓 学生管理 (add <学号> <姓名> <课程码> <班级> | import <csv> <课程码> <班级>)
    Student {
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
...
#[derive(Parser)]
#[command(name = "rhermes-stu")]
#[command(about = "RHermes 学生版 — 通用能力 + 学习流程", long_about = None)]
#[command(version)]
pub struct StudentCli {
    /// 恢复上一次会话内容
    #[arg(short = 'r', long)]
    resume: bool,

    #[command(subcommand)]
    command: Option<StudentCommands>,
}
...
/// 学生模式入口（rhermes-stu）
pub async fn run_student_app() {
    run_app(Some("student")).await;
}

/// 教师模式入口（rhermes-teacher）
pub async fn run_teacher_app() {
    run_app(Some("teacher")).await;
}

/// 统一应用入口（通用模式 + 角色模式共用）
///
/// - `role = None`：通用模式，默认行为是进入 TUI（受 config.edu 启用状态影响）
/// - `role = Some("student")`：学生模式，默认行为是进入学生认证/学习流程
/// - `role = Some("teacher")`：教师模式，默认行为是进入教师管理流程
///
/// 所有通用 CLI 子命令（init/config/mcp/gateway/debug）在三种模式下行为完全一致。
/// 教师/学生模式额外支持各自特有的一级子命令（如 course/login 等）。
async fn run_app(role: Option<&str>) {
    init_logging();
    ...
    // 根据 role 选择不同 Cli 解析器
    match role {
        Some("teacher") => {
            let cli = TeacherCli::parse();
            match cli.command {
                // 通用子命令
                Some(TeacherCommands::Common(common)) => {
                    handle_common_command(common, &config_path, needs_init, cli.resume, role).await;
                }
                // 教师特有子命令 → 透传给 edu::handle_edu
                Some(cmd) => {
                    let edu_args = teacher_cmd_to_edu_args(cmd);
                    edu::handle_edu("teacher", &edu_args, &config_path).await;
                }
                None => {
                    if needs_init {
                        run_init_or_die("[rhermes-teacher]");
                    }
                    println!("👩‍🏫 教师模式 (rhermes-teacher)");
                    edu::handle_edu("teacher", &[], &config_path).await;
                }
            }
        }
        Some("student") => {
            let cli = StudentCli::parse();
            match cli.command {
                // 通用子命令
                Some(StudentCommands::Common(common)) => {
                    handle_common_command(common, &config_path, needs_init, cli.resume, role).await;
                }
                // 学生特有子命令 → 透传给 edu::handle_edu
                Some(cmd) => {
                    let (edu_cmd, edu_args) = student_cmd_to_edu_args(cmd);
                    edu::handle_edu(edu_cmd, &edu_args, &config_path).await;
                }
                None => {
                    if needs_init {
                        run_init_or_die("[rhermes-stu]");
                    }
                    println!("🎓 学生模式 (rhermes-stu)");
                    edu::handle_edu("student", &[], &config_path).await;
                }
            }
        }
        _ => {
            ...
            match cli.command {
                ...
                None => {
                    if needs_init {
                        run_init_or_die("[RHermes]");
                    }
                    // 通用模式：检查是否启用教育模式（配置文件驱动）
                    let config = Config::load(&config_path).unwrap_or_default();
                    if config.edu.enabled {
                        let cfg_role = config.edu.role.as_str();
                        if !cfg_role.is_empty() {
                            println!("🎓 教育模式 ({cfg_role})");
                        }
                    }
                    run_code(cli.resume, None).await;
                }
            }
        }
    }
...
/// 初始化配置或退出
fn run_init_or_die(prefix: &str) {
    println!("📋 未检测到配置文件，正在启动初始化向导...");
    if let Err(e) = init::run_init() {
        eprintln!("{prefix} 初始化失败: {e}");
        std::process::exit(1);
    }
}

/// 教师特有命令 → edu::handle_edu 参数
///
/// 把 `rhermes-teacher course create CS101 数据结构` 转换为
/// `edu::handle_edu("teacher", &["course", "create", "CS101", "数据结构"], ...)`
fn teacher_cmd_to_edu_args(cmd: TeacherCommands) -> Vec<String> {
    match cmd {
        TeacherCommands::Common(_) => Vec::new(), // 不应到达此处
        TeacherCommands::Course { args } => prepend("course", args),
        TeacherCommands::Class { args } => prepend("class", args),
        TeacherCommands::Lesson { args } => prepend("lesson", args),
        TeacherCommands::Student { args } => prepend("student", args),
        TeacherCommands::Roster { code } => {
            let mut v = vec!["list".to_string()];
            if let Some(c) = code { v.push(c); }
            v
        }
...
/// 学生特有命令 → edu::handle_edu 的 (command, args)
///
/// 第一个元素是 edu::handle_edu 的 command 参数（不一定都是 "student"）：
/// - `rhermes-stu login 2024001 pwd` → handle_edu("auth", &["login", "2024001", "pwd"])
/// - `rhermes-stu join CS101`        → handle_edu("join", &["CS101"])
/// - `rhermes-stu status`            → handle_edu("status", &[])
/// - `rhermes-stu courses`           → handle_edu("courses", &[])
/// - `rhermes-stu profile`           → handle_edu("profile", &[])
/// - `rhermes-stu report`            → handle_edu("report", &[])
/// - `rhermes-stu mode explore`      → handle_edu("mode", &["explore"])
fn student_cmd_to_edu_args(cmd: StudentCommands) -> (&'static str, Vec<String>) {
    match cmd {
        StudentCommands::Common(_) => ("", Vec::new()), // 不应到达此处
        StudentCommands::Login { student_no, password } => {
            ("auth", vec!["login".to_string(), student_no, password])
        }
        StudentCommands::Join { code } => {
            ("join", vec![code])
        }
        StudentCommands::Status => ("status", Vec::new()),
        StudentCommands::Courses => ("courses", Vec::new()),
        StudentCommands::Profile => ("profile", Vec::new()),
        StudentCommands::Report => ("report", Vec::new()),
        StudentCommands::Mode { mode } => {
            let mut v = Vec::new();
...
/// 处理通用子命令（init/config/mcp/gateway/debug/edu）
///
/// `role` 仅用于 `edu` 子命令的默认行为（无参时）。
async fn handle_common_command(
    cmd: CommonCommands,
    config_path: &Path,
    _needs_init: bool,
    resume: bool,
    ...
    match cmd {
        ...
        CommonCommands::Edu { args } => {
            // 教育子命令：rhermes edu <student|teacher|auth|join|status> [...]
            // 第一个参数决定模式，其余透传给 handle_edu
            let (cmd, rest): (&str, Vec<String>) = match args.split_first() {
                Some((first, rest)) => (first.as_str(), rest.to_vec()),
                None => ("", Vec::new()),
            };
            // 如果是 rhermes-stu / rhermes-teacher 且没显式指定子命令，用 bin 默认角色
            let effective_cmd = if cmd.is_empty() {
                role.unwrap_or("")
            } else {
                cmd
            };
            edu::handle_edu(effective_cmd, &rest, config_path).await;
        }
    }
}
...
async fn run_code(resume: bool, role_hint: Option<&str>) {
    // 检测部署模式并初始化路径管理器
...
- skill_manage, memory, delegate_task, read_pdf\
        ...
        // 优先级：CLI role_hint > config.edu.role
        let edu_role = match role_hint {
            Some(r) if !r.is_empty() => r.to_string(),
            _ => config.edu.role.clone(),
        };
